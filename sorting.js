...
const intNums = strNums.map(stringsToNumbs);
console.log(intNums);
...
const evenNums = intNums.filter(getEvens);
console.log(evenNums);

const sumEvens = (total, numbers) => {
  return total + numbers;
}

const evenNumsSum = evenNums.reduce(sumEvens);
console.log(evenNumsSum);

// Find the index of the first value when added to it's index = 512 (#ATX!!)
const getAtxIdx = (element, index) => {
  let indexSum = index + index;
  if (indexSum == 512) {
      return index;
  }
};
const atxIdx = intNums.findIndex(getAtxIdx);
console.log(`index: ${atxIdx}, value: ${intNums[atxIdx]}`);
...
const weatherStatesNames = weather.map(item => item.weather_state_name);
const weatherStates = weatherStatesNames.filter((value, index, self) => self.indexOf(value) === index);
console.log(weatherStates);

//find the id of the object in weather that has a min_temp of 15.915
const getIdealTemp = (weather) => {
  if (weather.min_temp == 15.915) {
    return weather;
  }
}

const idealTemp = weather.find(getIdealTemp);
